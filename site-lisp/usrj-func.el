...
(defun usrj/common-keys ()
  ...
  (global-set-key (kbd "C-c b") emamux:keymap)
...
(defun usrj/keys ()
  ...
  (define-key mj-map (kbd "t") 'usrj/toggle-maximize-buffer)  
  ...
  (define-key mj-map (kbd "<down>") 'buf-move-down)
  (define-key mj-map (kbd "<up>") 'buf-move-up)
  (define-key mj-map (kbd "<left>") 'buf-move-left)
  (define-key mj-map (kbd "<right>") 'buf-move-right))
